...
namespace UttrekkFamilia
{
    internal class Uttrekk
    {
        ...
        public async Task ExtractSokratesAsync(BackgroundWorker worker, bool showResult = false)
        {
        ...
        private async Task<int> GetBVVBarnevernvaktsakerAsync(BackgroundWorker worker)
        {
            try
            {
                ...
                foreach (var caseCase in rawData)
                {
                    ...
                    sak.aktorId = GetActorId(await GetBVVPersonFromClientAsync(caseCase.ClientId));
        ...
        private async Task<int> GetBVVHenvendelserAsync(BackgroundWorker worker)
        {
            ...
            foreach (var enquiryDocument in rawEnquiryDocuments)
            {
                ...
                using (var context = new BVVDBContext(ConnectionStringVFB))
                {
                    henvendelse = await context.EnquiryEnquiries.Where(e => e.EnquiryEnquiryId == enquiryDocument.EnquiryId && e.CaseId.HasValue).FirstOrDefaultAsync();
                    if (henvendelse is null)
                    {
                        continue;
                    }
                }
                ...
                using (var context = new BVVDBContext(ConnectionStringVFB))
                {
                    CaseCase rawCase = await context.CaseCases.Where(e => e.CaseCaseId == henvendelse.CaseId).FirstOrDefaultAsync();
                    aktivitet.sakId = AddBydel(rawCase.Number, "SAK");
                }
                if (enquiryDocument.FileDataBlob != null)
                {
                    ...
                    if (WriteFiles)
                    {
                        await File.WriteAllBytesAsync(OutputFolderName + "filer\\" + document.filId + ".pdf", enquiryDocument.FileDataBlob);
                    }
                }
                else
                {
        ...
        private async Task<int> GetBVVJournalerAsync(BackgroundWorker worker)
        {
            ...
            foreach (var journalDocument in rawJournalDocuments)
            {
                ...
                using (var context = new BVVDBContext(ConnectionStringVFB))
                {
                    journal = await context.JournalJournals.Where(e => e.JournalJournalId == journalDocument.JournalId && e.CaseId.HasValue).FirstOrDefaultAsync();
                    if (journal is null)
                    {
                        continue;
                    }
                ...
                using (var context = new BVVDBContext(ConnectionStringVFB))
                {
                    CaseCase rawCase = await context.CaseCases.Where(e => e.CaseCaseId == journal.CaseId).FirstOrDefaultAsync();
                    aktivitet.sakId = AddBydel(rawCase.Number, "SAK");
                }
                if (journalDocument.FileDataBlob != null)
                {
                    Document document = new()
                    {
                        dokumentId = aktivitet.aktivitetId,
                    ...
                    if (WriteFiles)
                    {
                        await File.WriteAllBytesAsync(OutputFolderName + "filer\\" + document.filId + ".pdf", journalDocument.FileDataBlob);
                    }
                }
                else
                {
        ...
        private async Task<int> GetBVVPostAsync(BackgroundWorker worker)
        {
            ...
            foreach (var correspondenceDocument in rawCorrespondenceDocuments)
            {
                ...
                using (var context = new BVVDBContext(ConnectionStringVFB))
                {
                    correspondence = await context.CorrespondenceCorrespondences.Where(e => e.CorrespondenceCorrespondenceId == correspondenceDocument.CorrespondenceId && e.CaseId.HasValue).FirstOrDefaultAsync();
                ...
                using (var context = new BVVDBContext(ConnectionStringVFB))
                {
                    CaseCase rawCase = await context.CaseCases.Where(e => e.CaseCaseId == correspondence.CaseId).FirstOrDefaultAsync();
                    aktivitet.sakId = AddBydel(rawCase.Number, "SAK");
                }
                if (correspondenceDocument.FileDataBlob != null)
                {
                    Document document = new()
                    {
                        dokumentId = aktivitet.aktivitetId,
                    ...
                    if (WriteFiles)
                    {
                        await File.WriteAllBytesAsync(OutputFolderName + "filer\\" + document.filId + ".pdf", correspondenceDocument.FileDataBlob);
                    }
                }
                else
                {
        ...
        public async Task<string> GetSakerAsync(BackgroundWorker worker, bool meldingerUtenSakIsChecked)
        {
            try
            {
                ...
                int antall = await GetBarnevernsakerAsync(worker);
                if (meldingerUtenSakIsChecked)
                {
                    antall += await GetBarnevernsakerUtenSakAsync(worker);
                }
                antall += await GetTilsynssakerAsync(worker);
                antall += await GetOppdragstakersakerAsync(worker);
                return $"Antall saker: {antall}" + Environment.NewLine;
            }
            catch (Exception ex)
            {
                string message = $"Exception ({ex.Source}): {ex.Message} Stack trace: {ex.StackTrace}";
        ...
        private async Task<int> GetBarnevernsakerAsync(BackgroundWorker worker)
        {
            try
            {
                ...
                foreach (var klient in rawData)
                {
                    ...
                    sak.sakstype = "BARNEVERNSSAK";
                    saker.Add(sak);
                    if (mappings.HarTidligereBydeler(klient.KliLoepenr))
                    {
                        await GetDataTidligereBydelerAsync(klient.KliFoedselsdato.Value, klient.KliPersonnr.Value, sak.tidligereAvdelingListe);
                    }
                }
                if (MaksAntall > 0 && antall > MaksAntall)
                {
        ...
        private async Task<int> GetOppdragstakersakerAsync(BackgroundWorker worker)
        {
            try
            {
                ...
                foreach (var medarbeider in rawData)
                {
                    ...
                    if (antall % 10 == 0)
                    {
                        worker.ReportProgress(0, $"Behandler uttrekk oppdragssaker ({antall} av {totalAntall})...");
                    }
                    Sak sak = new()
                    {
                ...
                if (MaksAntall > 0 && antall > MaksAntall)
                {
                    ...
                    {
                        List<Sak> sakerPart = saker.Skip(toSkip).Take(MaksAntall).ToList();
                        await WriteFileAsync(sakerPart, GetJsonFileName("saker", $"Oppdragssaker{fileNumber}"));
                        fileNumber += 1;
                        toSkip += MaksAntall;
                    }
                }
                ...
                {
                    await WriteFileAsync(saker, GetJsonFileName("saker", "Oppdragssaker"));
                }
                return antall;
            }
            catch (Exception ex)
        ...
        public async Task<string> GetInnbyggereBarnAsync(BackgroundWorker worker, bool meldingerUtenSakIsChecked)
        {
            try
            {
                ...
                int antall = await GetInnbyggereBarnHovedAsync(worker);
                if (meldingerUtenSakIsChecked)
                {
                    antall += await GetInnbyggereBarnUtenSakAsync(worker);
                }
                return $"Antall barn: {antall}" + Environment.NewLine;
            }
            catch (Exception ex)
            {
                string message = $"Exception ({ex.Source}): {ex.Message} Stack trace: {ex.StackTrace}";
        ...
        public async Task<string> GetBarnetsNettverkBarnAsync(BackgroundWorker worker)
        {
            try
            {
                ...
                return $"Antall barnets nettverk - barnet: {antall}" + Environment.NewLine;
            }
            catch (Exception ex)
            {
                string message = $"Exception ({ex.Source}): {ex.Message} Stack trace: {ex.StackTrace}";
        ...
        public async Task<string> GetBarnetsNettverkAsync(BackgroundWorker worker)
        {
            try
            {
                ...
                return $"Antall barnets nettverk: {antall}" + Environment.NewLine;
            }
            catch (Exception ex)
            {
                string message = $"Exception ({ex.Source}): {ex.Message} Stack trace: {ex.StackTrace}";
        ...
        public async Task<string> GetMeldingerAsync(BackgroundWorker worker)
        {
            try
            {
                ...
                await UttrekkMeldingerAsync(false, rawData, meldinger, worker, totalAntall, "meldinger");
                ...
                return $"Antall meldinger: {rawData.Count}" + Environment.NewLine;
            }
            catch (Exception)
            {
                throw;
        ...
        private async Task UttrekkMeldingerAsync(bool utenSak, List<FaMeldinger> rawData, List<Melding> meldinger, BackgroundWorker worker, int totalAntall, string title)
        {
            int antall = 0;
        ...
        public async Task<string> GetMeldingerUtenSakAsync(BackgroundWorker worker)
        {
            try
            {
                ...
                List<Melding> meldinger = new();
                await UttrekkMeldingerAsync(true, rawData, meldinger, worker, totalAntall, "meldinger uten sak");
                if (MaksAntall > 0 && rawData.Count > MaksAntall)
                {
                    int toSkip = 0;
                    int fileNumber = 1;
                ...
                return $"Antall meldinger uten sak: {rawData.Count}" + Environment.NewLine;
            }
            catch (Exception ex)
            {
                string message = $"Exception ({ex.Source}): {ex.Message} Stack trace: {ex.StackTrace}";
        ...
        public async Task<string> GetUndersokelserAsync(BackgroundWorker worker)
        {
            try
            {
                ...
                foreach (var undersøkelse in rawData)
                {
                    ...
                    GetGrunnlagForTiltak(undersøkelse, undersoekelse);
                    //TODO Undersøkelser - utvidetFrist - If FA_UNDERSOEKELSE.UND_BEHANDLINGSTID = Null eller "1" -> False, "2" -> True og opprette Aktivitet av typen UTVIDELSE_AV_FRIST (undertype UNDERSØKELSE) + legg inn FA_UNDERSOEKELSE.UND_6MNDBEGRUNNELSE i activity.Notat
                    ...
                    if (undersøkelse.DokUplannr.HasValue)
                    {
                        ...
                        if (dokument != null)
                        {
                            ...
                            if (undersøkelse.ArkDokuplanDato.HasValue)
                            {
                                undersøkelsesplanAktivitet.hendelsesdato = undersøkelse.ArkDokuplanDato;
                                undersøkelsesplanAktivitet.utfortDato = undersøkelse.ArkDokuplanDato;
                            }
                            else
                            {
                                undersøkelsesplanAktivitet.hendelsesdato = undersøkelse.UndFerdigdatoUplan;
                                undersøkelsesplanAktivitet.utfortDato = undersøkelse.UndFerdigdatoUplan;
                            }
                            DocumentToInclude documentToInclude = new()
                            {
                    ...
                    if (undersøkelse.DokLoepenr.HasValue)
                    {
                        FaDokumenter dokument;
                        using (var context = new FamiliaDBContext(ConnectionStringFamilia))
                        {
                        ...
                        if (dokument != null)
                        {
                            ...
                            if (undersøkelse.ArkDoksrapportDato.HasValue)
                            {
                                undersøkelsesrapportAktivitet.hendelsesdato = undersøkelse.ArkDoksrapportDato;
                                undersøkelsesrapportAktivitet.utfortDato = undersøkelse.ArkDoksrapportDato;
                            }
                            else
                            {
                                undersøkelsesrapportAktivitet.hendelsesdato = undersøkelse.UndFerdigdato;
                                undersøkelsesrapportAktivitet.utfortDato = undersøkelse.UndFerdigdato;
                            }
                            DocumentToInclude documentToInclude = new()
                            {
                                dokLoepenr = dokument.DokLoepenr,
                ...
                return $"Antall undersøkelser: {antall}" + Environment.NewLine;
            }
            catch (Exception ex)
            {
                string message = $"Exception ({ex.Source}): {ex.Message} Stack trace: {ex.StackTrace}";
        ...
        public async Task<string> GetBrukereAsync(BackgroundWorker worker)
        {
            try
            {
                ...
                return $"Antall saksbehandlere: {antall}" + Environment.NewLine;
            }
            catch (Exception ex)
            {
                string message = $"Exception ({ex.Source}): {ex.Message} Stack trace: {ex.StackTrace}";
        ...
        public async Task<string> GetVedtakAsync(BackgroundWorker worker)
        {
            try
            {
                ...
                foreach (var saksJournal in rawData)
                {
                    ...
                    else if (saksJournal.SakBehFylkesnemnda == "FT")
                    {
                        vedtak.behandlingIFylkesnemda = "FULLTALLIG";
                    }
                    else if (saksJournal.SakBehFylkesnemnda == "SP")
                    {
                        //TODO Vedtak - behandlingIFylkesnemda - FA_SAKJOURNAL.SAK_BEH_FYLKESNEMNDA = "SP" opprette egen Aktivitet
                    }
                    ...
                    if ((saksJournal.SakAvgjortetat == "FN" || saksJournal.SakAvgjortetat == "LR" || saksJournal.SakAvgjortetat == "TR")
                        && (saksJournal.MynVedtakstype == "FN" || saksJournal.MynVedtakstype == "LA" || saksJournal.MynVedtakstype == "TI"))
                    {
                    ...
                    vedtak.aktivitetsUndertype = GetVedtakAktivitetsUnderType(saksJournal.MynVedtakstype, lovHovedParagraf, lovJmfParagraf1, lovJmfParagraf2, saksJournal.SakStatus);
                    ...
                    if (saksJournal.SakAvgjortetat == "FN" || saksJournal.SakAvgjortetat == "LR" || saksJournal.SakAvgjortetat == "TR" || saksJournal.MynVedtakstype == "FN" || saksJournal.MynVedtakstype == "LA" || saksJournal.MynVedtakstype == "TI")
                    {
                    ...
                    if (saksJournal.MynVedtakstype == "BV")
                    {
                ...
                return $"Antall vedtak: {antall}" + Environment.NewLine;
            }
            catch (Exception ex)
            {
                string message = $"Exception ({ex.Source}): {ex.Message} Stack trace: {ex.StackTrace}";
        ...
        public async Task<string> GetTiltakAsync(BackgroundWorker worker)
        {
            try
            {
                ...
                return $"Antall tiltak: {antall}" + Environment.NewLine;
            }
            catch (Exception ex)
            {
                string message = $"Exception ({ex.Source}): {ex.Message} Stack trace: {ex.StackTrace}";
        ...
        public async Task<string> GetPlanerAsync(BackgroundWorker worker)
        {
            try
            {
                ...
                return $"Antall planer: {antall}" + Environment.NewLine;
            }
            catch (Exception ex)
            {
                string message = $"Exception ({ex.Source}): {ex.Message} Stack trace: {ex.StackTrace}";
        ...
        public async Task<string> GetAktiviteterAsync(BackgroundWorker worker)
        {
            try
            {
                //TODO Aktiviteter - Legge inn Aktiviteten OPPDRAGSTAKER_AVTALE, SLETTET PostJournal, SLETTET Journal og OPPFØLGING (koble til Tiltak)
                worker.ReportProgress(0, "Starter uttrekk aktiviteter...");
                int antall = await GetTilsynsrapporterAsync(worker);
                antall += await GetInterneSaksforberedelserAsync(worker);
                antall += await GetVedtakAdministrativBeslutningerAktiviteterAsync(worker);
                antall += await GetJournalAktiviteterAsync(worker);
                antall += await GetIndividuellPlanAktiviteterAsync(worker);
                antall += await GetDokumentAktiviteterAsync(worker);
                return $"Antall aktiviteter: {antall}" + Environment.NewLine;
            }
            catch (Exception ex)
            {
                string message = $"Exception ({ex.Source}): {ex.Message} Stack trace: {ex.StackTrace}";
        ...
        private async Task<int> GetDokumentAktiviteterAsync(BackgroundWorker worker)
        {
            List<FaPostjournal> rawData;
            List<DocumentToInclude> documentsIncluded = new();
            ...
            foreach (var postjournal in rawData)
            {
                ...
                if (antall % 10 == 0)
                {
                    worker.ReportProgress(0, $"Behandler uttrekk dokumentaktiviteter ({antall} av {totalAntall})...");
                }
                if (!mappings.IsOwner(postjournal.KliLoepenr.Value))
                {
                    continue;
            ...
            if (MaksAntall > 0 && antall > MaksAntall)
            {
                ...
                {
                    List<Aktivitet> aktiviteterPart = aktiviteter.Skip(toSkip).Take(MaksAntall).ToList();
                    await WriteFileAsync(aktiviteterPart, GetJsonFileName("aktiviteter", $"Dokumentaktiviteter{fileNumber}"));
                    fileNumber += 1;
                    toSkip += MaksAntall;
                }
            }
            ...
            {
                await WriteFileAsync(aktiviteter, GetJsonFileName("aktiviteter", "Dokumentaktiviteter"));
            }
            return antall;
        }
        ...
        private async Task<int> GetJournalAktiviteterAsync(BackgroundWorker worker)
        {
            ...
            foreach (var journal in rawData)
            {
                ...
                {
                    if (journal.JotIdent == "HJ" || journal.JotIdent == "KS" || journal.JotIdent == "NO" || journal.JotIdent == "OT" || journal.JotIdent == "SB" || journal.JotIdent == "TO" || journal.JotIdent == "ET")
                    {
                    ...
                    else
                    {
                        ...
                        else if (journal.JotIdent == "OP")
                        {
                            ...
                            aktivitet.aktititetsUndertype = "FOSTERHJEMSBESØK";
                        }
                    }
                }
        ...
        private async Task<int> GetIndividuellPlanAktiviteterAsync(BackgroundWorker worker)
        {
            ...
            foreach (var plan in rawData)
            {
                ...
                aktivitet.tittel = "Individuell plan";
        ...
        private async Task GetDataTidligereBydelerAsync(DateTime fodselsDato, Decimal personNummer, List<TidligereAvdeling> tidligereAvdelinger)
        {
            ...
            try
            {
                foreach (var tidligereBydel in tidligereAvdelinger)
                {
                    ...
                    await GetMeldingerTidligereBydelAsync(fodselsDato, personNummer, tidligereBydel.avdelingId);
        ...
        private async Task GetMeldingerTidligereBydelAsync(DateTime fodselsDato, Decimal personNummer, string bydel)
        {
            try
            {
                List<FaMeldinger> rawData;
                ...
                foreach (var meldingFamilia in rawData)
                {
                    ...
                    melding.mottattBekymringsmelding = GetMottattBekymringsmelding(meldingFamilia, bydel);
                    melding.behandlingAvBekymringsmelding = GetBehandlingAvBekymringsmelding(meldingFamilia, bydel);
                    melding.tilbakemeldingTilMelder = await GetTilbakemeldingTilMelderAsync(meldingFamilia, bydel);
                    meldinger.Add(melding);
                }
        ...
        private async Task<string> GetDocumentsAsync(BackgroundWorker worker, string sourceName, List<DocumentToInclude> documentsIncluded)
        {
            try
            {
        ...
        private async Task GetHtmlDocumentsAsync(BackgroundWorker worker, List<HtmlDocumentToInclude> htmlDocumentsIncluded, string category, string abbreviation)
        {
            try
            {
        ...
        private static void GetGrunnlagForTiltak(FaUndersoekelser undersøkelse, Undersøkelse undersoekelse)
        {
